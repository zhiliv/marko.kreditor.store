...
router.get('/', async (ctx, next) => {
	...
	let chkTypeDevice = {
		isTablet: require('util').inspect(ctx.userAgent.isTablet),
		isMobile: require('util').inspect(ctx.userAgent.isMobile),
		isDesktop: require('util').inspect(ctx.userAgent.isDesktop)
	};
	let listCategory;
	await fn.getTypeCategory().then(res => {
		listCategory = res;
	});
	//тип изобрадения
	let typeImg = fn.getTypeImg(chkTypeImg);
	let typeDevice = fn.getTypeDevice(chkTypeDevice);
	//заполнение содеожимого
	//параметр navbar - параметры активного меню
	ctx.body = template.main.stream({ navbar: '/', typeImg: typeImg, listCategory: listCategory, typeDevice: typeDevice });
});
